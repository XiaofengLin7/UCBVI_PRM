...
def run_exp(epi_len, num_epi, num_states, num_exp, test_name="river_swim_patrol"):
    ...
    # learner_1 = UCBVI_RM(n_states, n_actions, epi_len, delta=0.05, K=num_epi, RM=env.rewardMachine, bonus_scale=1e-3)
    learner_1 = UCRL2_RM_Bernstein(n_states, n_actions, epi_len, delta=0.05, K=num_epi, RM=env.rewardMachine)
    learner_2 = UCRL2_RM(n_states, n_actions, epi_len, delta=0.05, K=num_epi, RM=env.rewardMachine, distance_scale=0.35)
    # learner_3 = UCBVI_RM(n_states, n_actions, epi_len, delta=0.05, K=num_epi, RM=env.rewardMachine, bonus_scale=1e-3)
    # learner_2 = UCBVI_RM(n_states, n_actions, epi_len, delta=0.05, K=num_epi, RM=env.rewardMachine, bonus_scale=1e-8)
    # learner_2 = UCBVI_CP(env.rewardMachine.n_states, n_states, n_actions, epi_len, delta=0.05, K=num_epi, rm_rewards=env.rewardMachine.rewards, bonus_scale=0.01)
    ...
    if save_data is True:
        ...
        if check_file and os.path.exists(data_path):
            ...
            np.savez(dir_name + data_name, data_1=data_1, data_2=data_2, V_star=V_star, P_star=P_star)
...
def data_collect(env, learner_1, learner_2, epi_len, i):
    ...
    # res1 = 0
    # res2 = [np.zeros(epi_len) for x in res1]
...
if __name__ == "__main__":
    ...
    if data_visualization:
        ...
        # plt.plot(k, V_star * np.ones(num_epi), marker='.', color='g', label='V*')
        # em_cumu_reward_star = mean_cumu_reward_star
        # em_cumu_reward_star[1:] = mean_cumu_reward_star[1:] - mean_cumu_reward_star[:-1]
        # plt.plot(k, em_cumu_reward_star.mean() * np.ones(num_epi), marker='.', color='purple', label='empirical_optimal')
        # plt.legend()
        # plt.xlim(visual_range)
        ...
        interval = 200
        mean_reward_per_episode_1 = mean_cumu_reward_1
        mean_reward_per_episode_1[1:] = mean_cumu_reward_1[1:] - mean_cumu_reward_1[:-1]
        mean_reward_per_episode_2 = mean_cumu_reward_2
        mean_reward_per_episode_2[1:] = mean_cumu_reward_2[1:] - mean_cumu_reward_2[:-1]
        mean_reward_per_episode_1 = mean_of_every_interval(mean_reward_per_episode_1, interval)
        mean_reward_per_episode_2 = mean_of_every_interval(mean_reward_per_episode_2, interval)
        plt.figure(4)
        k = np.arange(num_epi/interval)
        plt.plot(k*interval, mean_reward_per_episode_1, marker='.', color='b', label=learner_1.name())
        plt.plot(k*interval, mean_reward_per_episode_2, marker='.', color='r', label=learner_2.name())
        # plt.figure(4)
        # plt.plot(k, mean_reward_per_episode_1, marker='.', color='b', label=learner_1.name())
        # plt.plot(k, mean_reward_per_episode_2, marker='.', color='r', label=learner_2.name())
        plt.plot(k*interval, V_star * np.ones((int)(num_epi/interval)), marker='.', color='g', label='V*')
        plt.legend()
        plt.xlim(visual_range)
